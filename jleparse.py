...
custom_header = b'\x02\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x01\x14\x02\x00\x00\x00\x00\x00\xC0\x00\x00\x00\x00\x00\x00\x46'
lnk_guid = b'\x4C\x00\x00\x00\x01\x14\x02\x00\x00\x00\x00\x00\xC0\x00\x00\x00\x00\x00\x00\x46'
lnk_guid_size = 20
...
def main(argv):
	...
	if(jle_type == "a"):
		...
		if(file_sig != auto_header):
			print("The specified file is not an automatic jumplist")
			exit()
		...
		if(file_sig != custom_header):
			print("The specified file is not a custom jumplist")
			exit()
		lnk_exists = True
		lnk_offset = 0
		jle_file.seek(0)
